...
# 磁盘预读
# -- 内存池跟磁盘进行交互的时候,有一个最基本的逻辑单位称为:页,也叫datapage,大小一般是4k或者8k,我们进行数据读取的时候,一般读的是页的整数倍.
...
# 数据更新流程
# 1.执行器先从引擎中找到数据,如果数据在内存中直接返回,如果不存在内存中,查询后返回
...
- 粒度分类
  ...
  - 记录锁(`Record Lock`)
    ...
    - ```json
      # 记录锁也属于行锁当中的一种,只不过记录锁的范围只是表中的某一条记录,记录锁是说事务在加锁后锁住的只是表的某一条记录,加了记录锁之后数据可以避免数据在查询的时候被修改的重复读问题,也避免了在修改的事务未提交前,被其他事务读取的脏读问题
      ```
  ...
  - 临键锁(`Next-Key Lock`)
    ...
    - ```json
      ...
      # 总结来说,就是记录锁和间隙锁的组合,临键锁会把查询出来的记录锁住,同时也会把该范围查询内的所有间隙空间也锁住,再之它会把相邻的下一个区间也锁住.
      ```
...
> 在进行索引设计的时候,应该保证索引字段占用的空间越小越好,还有以下的一些细节需要注意：

- 1.适合索引的列是出现在 `WHERE` 子句中的列,或者链接子句中的列;
- 2.基数较小的表,索引效果差,没必要创建索引;
- 3.在选择索引列的时候,越短越好,可以指定某些列的一部分,没必要用全部字段的值;
- 4.不要给表中的每一个字段创建索引,并不是索引越多越好;
- 5.定义有外键的数据列一定要创建索引;
- 6.更新频繁的字段不要有索引;
- 7.创建索引的列不要过多,可以创建组合索引,但是组合索引的列的个数也不建议太多;
- 8.大文本,大对象不要创建索引;
...
- 1.从库通过手动执行 `change master to` 语句连接主库,主库提供了连接用户的一切条件(`username`,`password`,`ip`,`port`),并且让从库知道二进制日志的起点位置(`file` 名和 `position`号 ); start slave;
- 2.从库的I/O线程和主库的dump线程建立连接;
- 3.从库I/O线程根据 `change master to` 语句提供的 `file`名 `position` 号,向主库发起 `binlog` 的请求;
- 4.主库 dump 线程根据从库的请求,将本地的 `binlog` 以 `events` 的方式发给从库I/O线程;
- 5.从库 I/O 线程接收 `binlog events`,并存放到本地 `relay-log` 中,传送过来的信息会记录到 `master.info` 中;
- 6.从库 `SQL` 线程应用 `relay-log` ,并且把应用过的记录记录到 `relay-log.info` 中,默认情况下,已经应用过的`relay-log`会被自动清理掉(`purge`).
