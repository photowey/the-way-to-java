...
- 1.从库通过手动执行 `change master to` 语句连接主库,主库提供了连接用户的一切条件(`username`,`password`,`ip`,`port`),并且让从库指导二进制日志的起点位置(`file` 名和 `position`号 ); start slave;
...
- 5.从库 I/O 线程接收 `binlog events`,并存放到本地 `relay-log` 中,传送过来的信息或记录到 `master.info` 中;
