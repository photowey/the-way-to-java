 ...
 */
package com.photowey.jvm.delayed.queue.config;

import com.photowey.jvm.delayed.queue.event.DelayedEvent;
import com.photowey.jvm.delayed.queue.handler.DefaultDelayedQueueHandler;
import com.photowey.jvm.delayed.queue.handler.DelayedQueueHandler;
import com.photowey.jvm.delayed.queue.listener.DelayedQueueListener;
import com.photowey.jvm.delayed.queue.publisher.DefaultDelayedQueueEventPublisher;
import com.photowey.jvm.delayed.queue.publisher.DelayedQueueEventPublisher;
 ...
 */
@Configuration
@Import(value = {DelayedQueueListenerRegistry.class})
public class DelayedQueueAutoConfigure implements BeanFactoryAware, SmartInitializingSingleton {

    @Autowired
    private DelayedQueueListenerRegistry listenerRegistry;
    ...
    public void afterSingletonsInstantiated() {
        ...
        beans.forEach((k, listener) -> this.listenerRegistry.registerListener(listener));
    }
    ...
    @Bean
    @ConditionalOnMissingBean
    public DelayedQueueHandler delayedQueueHandler(DelayQueue<DelayedEvent> delayQueue,
                                                   @Qualifier("delayedTaskExecutor") ThreadPoolTaskExecutor delayedTaskExecutor) {
        return new DefaultDelayedQueueHandler(delayQueue, delayedTaskExecutor, this.listenerRegistry);
    }
