...
import com.photowey.plugin.xcurl.lang.XCURLLexerAdapter;
...
/**
 * {@code XCURLSyntaxHighlighter}
 *
 * @author photowey
 * @date 2022/11/15
 * @since 1.0.0
 */
public class XCURLSyntaxHighlighter extends SyntaxHighlighterBase {
    ...
    TextAttributesKey[] getTokenHighlights(IElementType tokenType) {
        if (tokenType.equals(XCURLTypes.CURL) || tokenType.equals(XCURLTypes.OPTION) || tokenType.equals(XCURLTypes.METHOD)) {
            return KEYWORD_KEYS;
        }
        if (tokenType.equals(XCURLTypes.QUOTED_STRING) || tokenType.equals(XCURLTypes.BASIC_STRING)) {
            return STRING_KEYS;
        }
        if (tokenType.equals(XCURLTypes.COMMENT)) {
            return COMMENT_KEYS;
        }
